...
// about us team animations
const teamPeopleTm = gsap.timeline()
const aboutUsTeam = document.querySelector('[about-us-team]')
const teamPeople = document.querySelectorAll('[team-person]')

function animateTeamPeople(){
    teamPeople.forEach((teamPerson) => {
        teamPeopleTm.to(teamPerson, {x:0, opacity:1})
    })
}

observeElement(aboutUsTeam, animateTeamPeople)

// agency text animations
const agencyTm = gsap.timeline({defaults:{opacity:0}})
...
function animateTexts(){
    agencyTm.fromTo(agencyMainText, {y:20}, {opacity:1, y:0})
    agencyTm.fromTo(agencySubText, {y:-20}, {opacity:1, y:0})
}
